...
with open('./mpi1.csv') as mpi1, open('./mpi2.csv') as mpi2, open('./mpi3.csv') as mpi3, \
    ...
    for row in reader1:
        if row[0] == '1' and round(Decimal(row[2]), 1) != 2.2:
            if round(Decimal(row[2]), 1) in rank1:
                ...
                rank1[round(Decimal(row[2]), 1)] = {int(row[1]): Decimal(row[3]) + ntp_shift['mpi1']}
        elif row[0] == '2' and round(Decimal(row[2]), 1) != 2.2:
            if round(Decimal(row[2]), 1) in rank2:
                if int(row[1]) in rank2[round(Decimal(row[2]), 1)]:
                    rank2[round(Decimal(row[2]), 1)][int(row[1])] = (Decimal(row[3]) + ntp_shift['mpi1'])
                else:
                    rank2[round(Decimal(row[2]), 1)][int(row[1])] = Decimal(row[3]) + ntp_shift['mpi1']
            else:
                rank2[round(Decimal(row[2]), 1)] = {int(row[1]): Decimal(row[3]) + ntp_shift['mpi1']}
    ...
    for row in reader3:
        ...
        if round(float(row[2]), 1) == 2.2:
            break
        while i < len(rank2[round(Decimal(row[2]), 1)]):
            temp = ((Decimal(row[3]) + ntp_shift['mpi2']) - rank1[round(Decimal(row[2]), 1)][i])
            if temp < 0:
                continue
            elif round(Decimal(row[2]), 1) in delta:
                delta[round(Decimal(row[2]), 1)].append(
                    (Decimal(row[3]) + ntp_shift['mpi3'] - rank2[round(Decimal(row[2]), 1)][i]))
            else:
                delta[round(Decimal(row[2]), 1)] = [
                    (Decimal(row[3]) + ntp_shift['mpi3'] - rank2[round(Decimal(row[2]), 1)][i])]
            i += 1
